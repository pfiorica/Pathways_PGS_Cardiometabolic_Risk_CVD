...
#pheno_list<- paste0(pheno_list, "_time")
...
# Assuming your data is in a dataframe called 'combined_data'
...
for (var in categorical_vars){
  covariate_file[[var]] <- as.factor(covariate_file[[var]])
}
...
characterization <- c("CMD", "CMD", "CMD" , "CMD", "CVD", "CVD", "CVD", "CVD" , "CVD", "CVD", "CVD", "CVD")
...
for(pheno in pheno_list$pheno_list){
  ...
  XWAS_res <- xwas(df=df,
                   ...
                   IDA = group_a_IDs,
                   removes = REM)
  fwrite(XWAS_res, out_path %&% "/pxs_output/cox_" %&% pheno %&% "_pxstools_" %&% length(XVAR)  %&%"_variables_out.txt",  col.names = T, row.names = T, sep = "\t", quote = F)
  ...
  XWAS_pgs <- xwas(df=df_pgs,
                   ...
                   IDA = group_a_IDs,
                   removes = REM)
  fwrite(XWAS_res, out_path %&% "/pxs_output/cox_" %&% pheno %&% "_pxstools_" %&% length(XVAR)  %&%"_variables_out_pgs.txt",  col.names = T, row.names = T, sep = "\t", quote = F)
  ...
  print("Calculating C-index for PXS ...")
  
  roc_obj <- roc(PXSS$TIME, PXSS$pred)
  auc_value <- auc(roc_obj)
  print("PXS AUC of ROC " %&% pheno %&% ": " %&% auc_value)
  
  data <- data.frame(pred = PXSS$pred, phenotype = PXSS$TIME)
  ...
  roc_obj_pgs <- roc(PXS_pgs$TIME, PXS_pgs$pred)
  auc_value_pgs <- auc(roc_obj_pgs)
  print("PXS AUC of ROC " %&% pheno %&% ": " %&% auc_value_pgs)
  
  data_pgs <- data.frame(pred = PXS_pgs$pred, phenotype = PXS_pgs$TIME)
  ...
  # Save plot
  png(out_path %&% "/pxs_output/cox_" %&% pheno %&% "_roc_curve.png")
  plot(
    roc_obj,
    col = "blue",
