...
for (var in treatment){
  for (val in c(0,1)){
    ...
    strat_pheno <- pheno[pheno[[var]]==val]
    covariates_minus_var <- setdiff(covariates, var)
    for (i in outcomes){
      a <- strat_pheno %>%
      ...
      fwrite(b, dir %&% "ALL_" %&% i %&% "_" %&% var %&% "_" %&% val %&% "_covariates.txt", col.names = T, row.names = F, sep = "\t", quote = F )
      fwrite(a, dir %&% "ALL_" %&% i %&% "_" %&% var %&% "_" %&% val %&% "_phenotype.txt",  col.names = T, row.names = F, sep = "\t", quote = F)
    }
  }
}
