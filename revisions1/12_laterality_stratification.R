...
lat_data <- readRDS("/bd-fs-mnt/TenantShare/exthome/home/l578311/cvd/cvd/pgs_cohort_tumor_clean_laterality.rds")
...
pheno <- pheno %>%
  ...
  left_join(lat_data, by = c("STUDYID"="studyid"))
...
treatment<- c("rad_tx_yn")
for (side in laterality){
  for (var in treatment){
    for (val in c(0,1)){
      # Write individual phenotype files
      strat_pheno <- pheno[pheno[[var]]==val] # subset to individuals who did not did not receive a given treatment
      strat_pheno <- strat_pheno %>% filter(laterality  == side) #Filter to individuals who had a certain sided tumor
      covariates_minus_var <- setdiff(covariates, var) # Remove Radiation from the list of covariates
      for (i in outcomes){
        a <- strat_pheno %>%
        ...
        a <- a %>% filter(!!sym(i) %in% c("0","1")) # Ensure phenotype/outcome value is 0 or 1
        b <- strat_pheno %>% # Make covariate file
          mutate(IID=ID, FID=ID) %>%
          select(IID, FID, all_of(covariates_minus_var), ancestry)  %>%
        ...
        fwrite(b, dir %&% "ALL_" %&% i %&% "_" %&% side %&% "_" %&% var %&% "_" %&% val %&% "_covariates.txt", col.names = T, row.names = F, sep = "\t", quote = F )
        fwrite(a, dir %&% "ALL_" %&% i %&% "_" %&% side %&% "_" %&% var %&% "_" %&% val %&% "_phenotype.txt",  col.names = T, row.names = F, sep = "\t", quote = F)
      }
    }
  }
}
